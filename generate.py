from utils.engine import DDPMSampler, DDIMSampler, DPMSolverSampler
...
def parse_option():
    ...
    parser.add_argument("--sampler", type=str, default="ddim", choices=["ddpm", "ddim","dpm"])
    ...
    # DPM Solver sampler param
    parser.add_argument("--solver_order", type=int, default=1)
...
def generate(args):
    ...
    if args.sampler == "ddim":
        ...
        sampler = DPMSolverSampler(model, **cp["config"]["Trainer"]).to(device)
    else:
    ...
    extra_param = dict(steps=args.steps, eta=args.eta, method=args.method, solver_order = args.solver_order)
